...
const CheckFilled = ({ className }: { className?: string }) => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className={clsx("w-6 h-6 ", className)}>
      <path
        fillRule="evenodd"
    ...
    <div className="flex relative justify-start max-w-xl mx-auto flex-col mt-40">
      {loadingStates.map((loadingState, index) => {
        ...
        return (
          <motion.div
            key={index}
            className={clsx("text-left flex gap-2 mb-4")}
          ...
          >
            <div>
              ...
              {index <= value && <CheckFilled className={clsx("text-black dark:text-white", value === index && "text-black dark:text-lime-500 opacity-100")} />}
            </div>
            <span className={clsx("text-black dark:text-white", value === index && "text-black dark:text-lime-500 opacity-100")}>{loadingState.text}</span>
          </motion.div>
        )
      })}
    </div>
