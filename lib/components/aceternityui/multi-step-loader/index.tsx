    ...
    <div className="relative mx-auto mt-40 flex max-w-xl flex-col justify-start">
      {loadingStates.map((loadingState, index) => {
        ...
        return (
          <motion.div
            key={index}
            className={clsx("mb-4 flex gap-2 text-left")}
    ...
    <AnimatePresence mode="wait">
      {loading && (
        ...
        >
          ...
          <div className="absolute inset-x-0 bottom-0 z-20 h-full bg-white bg-linear-to-t [mask-image:radial-gradient(900px_at_center,transparent_30%,white)] dark:bg-black" />
        </motion.div>
      )}
    </AnimatePresence>
  )
