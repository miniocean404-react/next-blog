...
<div>
  ...
  <span className="mix-blend-plus-lighter text-zinc-400/80">is a Rehype plugin powered by the</span>
  [`shiki`](https://github.com/shikijs/shiki){" "}
  <span className="mix-blend-plus-lighter text-zinc-400/80">
    syntax highlighter that provides beautiful code blocks for Markdown or MDX. It works on both the
    server at build-time (avoiding runtime syntax highlighting) and on the client for dynamic
    highlighting.
  </span>
</div>

## Editor-Grade Highlighting 3
...
```js file=<rootDir>/lib/content/components/skeleton.tsx
...
The following works both on the server and on the db().
